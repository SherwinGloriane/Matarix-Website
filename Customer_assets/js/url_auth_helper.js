...
(function() {
    ...
    /**
     * Check if user is logged in (has user_id in URL AND in sessionStorage)
     * @returns {boolean} True if user is actually logged in
     */
    function isLoggedIn() {
        const userIdInUrl = getUserId();
        const userIdInStorage = sessionStorage.getItem('user_id');
        
        // User is only considered logged in if BOTH URL and sessionStorage have user_id
        // This prevents preserving user_id when user is not actually logged in
        return userIdInUrl !== null && userIdInUrl !== '' && userIdInStorage !== null && userIdInStorage !== '';
    }
    ...
    function addUserIdToUrl(url) {
        // Only add user_id if user is actually logged in (check sessionStorage)
        const userIdInStorage = sessionStorage.getItem('user_id');
        if (!userIdInStorage || !url) {
            return url; // No user_id in storage or no URL
        }
        
        const userId = userIdInStorage;
    ...
    /**
     * Remove user_id from current URL
     */
    function removeUserIdFromCurrentUrl() {
    ...
    /**
     * Clean up URL - remove user_id if user is not actually logged in
     */
    function cleanupUrl() {
        const urlParams = new URLSearchParams(window.location.search);
        const userIdInUrl = urlParams.get('user_id');
        const userIdInStorage = sessionStorage.getItem('user_id');
        
        // If user_id is in URL but not in sessionStorage, remove it
        if (userIdInUrl && !userIdInStorage) {
            console.log('Cleaning up URL: user_id found in URL but user is not logged in. Removing user_id from URL.');
            removeUserIdFromCurrentUrl();
        }
    }
     ...
     */
    function init() {
        // First, clean up URL if user is not actually logged in
        // Use multiple timeouts to ensure it runs after customer_auth_check.js
        setTimeout(function() {
            cleanupUrl();
        }, 100);
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function() {
                cleanupUrl(); // Clean up again after DOM loads
        ...
        if (typeof jQuery !== 'undefined') {
            jQuery(document).ready(function() {
                cleanupUrl(); // Clean up again after jQuery ready
                preserveUserIdInLinks();
        ...
        console.log('URL Auth Helper initialized. User ID in URL:', userId, 'Actually logged in:', isActuallyLoggedIn);
    }
